...
- **Status:** desenho aprovado pelo dono; ajustes dos reviews aplicados — plano v3 commitado (`docs/plans/2026-09-21-atualizacao-automatica.md`); implementação pendente
...
- `main.js:53` (`require("./updater")`), `main.js:2510-2518` (função `procurarAtualizacao`, chamada no `did-finish-load` em `main.js:2910` e respeitando `check_updates_on_start`; o boot em si é `main.js:2910`/`main.js:2959`), `main.js:4118-4142` (IPC `update:state` / `update:check` / `update:apply`)
...
| 6 | UI | `app/src/components/UpdateDialog.tsx` (reuso) + `app/src/components/desktop/GeneralSection.tsx` (toggle/botão existentes) | estados novos: "disponível", "baixando (N%)", "pronto" |
...
- O `release/` passa a acumular yml a partir do primeiro build com `publish`: o script de release precisa **apagar os yml antes do build** e **usar o yml da versão que está sendo publicada**, nunca o mais antigo que estiver na pasta.
...
| Usuário escolhe "Depois" **depois** de baixar | o update fica pendente no cache (`update_pendente_versao`); o aviso "Pronto — Reiniciar agora" reaparece no próximo boot; o arquivo **completo** em cache é reaproveitado (o electron-updater 6 descarta download parcial — não há retomada byte a byte; NSIS e AppImage usam download diferencial quando o `.blockmap` está publicado). Com o toggle `check_updates_on_start` desligado não há checagem de boot, então o reaviso de "pronto" só sai na checagem manual em Configurações |
...
| D6 | "Depois" pós-download, cache, retomada | pendente no cache (`update_pendente_versao`), reavisa no boot, revalida o arquivo **completo** em cache (sem retomada byte a byte na v6); com o toggle desligado, só na checagem manual |
...
- Manual (documentado): AppImage 1.4.1 → publicar 1.4.2 → ver o diálogo, baixar, reiniciar, conferir a versão; desligar `check_updates_on_start`, reabrir e conferir que o diálogo automático não abre, mas "Procurar atualizações" funciona (D10).
