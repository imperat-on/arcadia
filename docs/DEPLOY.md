...
O arquivo `server/systemd/arcadia-server.service` fornece um exemplo de unidade systemd. Ajuste `User`, `WorkingDirectory` e `EnvironmentFile` para a maquina de deploy. Exponha o servico apenas por HTTPS, por exemplo via Tailscale Funnel, e configure `ARCADIA_SUPABASE_URL` no cliente Electron para a URL publica.
