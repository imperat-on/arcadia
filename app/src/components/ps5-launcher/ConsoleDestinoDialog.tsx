...
//
// O overlay vem do Modal; o foco inicial vai para o primeiro destino e é
// refeito quando a lista chega (focusKey), porque ela chega async.
export function ConsoleDestinoDialog({
  ...
  return (
    <Modal
      ...
      size="lg"
      showClose={false}
      initialFocus="button[data-destino]"
      focusKey={opcoes.length}
      footer={
        <button
          onClick={onFechar}
          className="w-full rounded-lg border border-white/10 py-2.5 text-[13px] text-white/55 outline-none transition-colors hover:text-white/85"
        >
          {t("common.cancelar")}
        </button>
      }
