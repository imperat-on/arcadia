...
interface TopBarProps { profile?: Profile; activeTab:number; onTab:(i:number)=>void; onRefresh:()=>void; onOpenDownloads:()=>void; onOpenProfile:()=>void; onOpenAbout:()=>void; menuOpen:boolean; onToggleMenu:()=>void; onCloseMenu:()=>void; showHidden:boolean; onToggleShowHidden:()=>void; libraryFilter?:LibraryFilter; onLibraryFilter?:(f:LibraryFilter)=>void; search?:string; onSearch?:(v:string)=>void }
...
export const TABS=["topbar.noticias","topbar.jogos","store.titulo"]
export function TopBar({profile,activeTab,onTab,onRefresh,onOpenDownloads,onOpenProfile,onOpenAbout,menuOpen,onToggleMenu,onCloseMenu,showHidden,onToggleShowHidden,libraryFilter="all",onLibraryFilter,search="",onSearch}:TopBarProps){
 ...
 return <header className="ps5-topbar anim-nav fixed inset-x-0 top-0 z-30 flex h-[92px] items-center px-[4.5vw] text-white">
  <nav className="ps5-primary-nav flex items-center gap-9" aria-label={t("topbar.nav_aria")}>{TABS.map((key,i)=>({i,l:t(key)})).map(x=><button key={x.i} data-active={activeTab===x.i} onClick={()=>onTab(x.i)}>{x.l}</button>)}</nav>
  <div className="ml-auto flex items-center gap-5">{activeTab===1&&<label className="ps5-search"><Icon className="h-5 w-5"><circle cx="11" cy="11" r="7"/><path d="m20 20-4-4"/></Icon><input aria-label={t("topbar.buscar")} value={search} onChange={e=>onSearch?.(e.target.value)} placeholder={t("topbar.buscar")}/></label>}
   <button type="button" className="console-download-action" onClick={onOpenDownloads} aria-label={t("downloads.titulo")}>↓</button>
   <time className="ps5-time">{now.toLocaleTimeString(userLocale(),{hour:"2-digit",minute:"2-digit"})}</time>
   <div className="relative"><button type="button" aria-label={t("topbar.perfil")} aria-expanded={menuOpen} onClick={onToggleMenu} className="ps5-profile-button">{profile?.avatar?<img src={profile.avatar} alt=""/>:initial}</button><UserMenu open={menuOpen} onClose={onCloseMenu} onOpenProfile={onOpenProfile} onOpenAbout={onOpenAbout} onRefresh={onRefresh} showHidden={showHidden} onToggleShowHidden={onToggleShowHidden} profile={profile}/></div>
  </div>
  {activeTab===1 && <nav className="console-filters" aria-label={t("topbar.jogos")}>{(["all","favorites","collections"] as LibraryFilter[]).map(filter=><button type="button" key={filter} aria-pressed={libraryFilter===filter} onClick={()=>onLibraryFilter?.(filter)}>{t(`console.filter.${filter}`)}</button>)}</nav>}
 </header>
}
