...
interface TopBarProps { profile?: Profile; activeTab:number; onTab:(i:number)=>void; onRefresh:()=>void; onOpenSettings:()=>void; onOpenProfile:()=>void; menuOpen:boolean; onToggleMenu:()=>void; onCloseMenu:()=>void; showHidden:boolean; onToggleShowHidden:()=>void; libraryFilter?:LibraryFilter; onLibraryFilter?:(f:LibraryFilter)=>void; search?:string; onSearch?:(v:string)=>void }
...
export function TopBar({profile,activeTab,onTab,onRefresh,onOpenSettings,onOpenProfile,menuOpen,onToggleMenu,onCloseMenu,showHidden,onToggleShowHidden,libraryFilter="all",onLibraryFilter,search="",onSearch}:TopBarProps){
 ...
 return <header className="ps5-topbar anim-nav fixed inset-x-0 top-0 z-30 flex h-[92px] items-center px-[4.5vw] text-white">
  ...
  <div className="ml-auto flex items-center gap-5">{activeTab===1&&<label className="ps5-search"><Icon className="h-5 w-5"><circle cx="11" cy="11" r="7"/><path d="m20 20-4-4"/></Icon><input value={search} onChange={e=>onSearch?.(e.target.value)} placeholder="Buscar"/></label>}
   ...
   <div className="relative"><button onClick={onToggleMenu} className="ps5-profile-button">{profile?.avatar?<img src={profile.avatar} alt=""/>:initial}</button><UserMenu open={menuOpen} onClose={onCloseMenu} onOpenSettings={onOpenSettings} onOpenProfile={onOpenProfile} onRefresh={onRefresh} showHidden={showHidden} onToggleShowHidden={onToggleShowHidden} profile={profile}/></div>
  </div>
 </header>
}
