      ...
      {verCategorias ? (
        <>
          <button
            ...
            className="flex w-full items-center gap-2 px-4 py-2 text-[12px] text-white/50 transition-colors hover:text-white"
          >
          ...
          <div className="max-h-[220px] overflow-y-auto">
            {allCategories.length === 0 && (
              <p className="px-4 py-2 text-[12px] text-white/35">{t("library.sem_categorias")}</p>
            )}
            {allCategories.map((c) => (
              <label
                key={c}
                className="flex cursor-pointer items-center gap-3 px-4 py-2 text-[13px] text-white/85 transition-colors hover:bg-white/[0.06]"
              >
                <input
                  type="checkbox"
        ...
        <>
          {game.installed !== false ? (
            <Item
              onClick={run(actions.jogar)}
              icon={I("M8 5v14l11-7z")}
              label={t("library.jogar")}
            />
          ) : (
            <Item
              onClick={run(actions.jogar)}
              icon={I("M5 20h14v-2H5v2zM19 9h-4V3H9v6H5l7 7 7-7z")}
              label={t("library.instalar")}
            />
          )}
          <Item
            onClick={run(actions.detalhes)}
            icon={I(
              "M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8l-6-6zm2 16H8v-2h8v2zm0-4H8v-2h8v2zm-3-5V3.5L18.5 9H13z",
            )}
            label={t("library.detalhes")}
          />
          <Item
            onClick={run(actions.configuracoes)}
            icon={I(
          ...
          <Item
            ...
            icon={I(
              "M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8l-6-6zm-1 7V3.5L18.5 9H13zM8 12h8v2H8v-2zm0 4h8v2H8v-2z",
            )}
            label={t("library.registros")}
          />
          <Item
            onClick={run(actions.editar)}
            icon={I(
            ...
            label={t("library.editar_jogo")}
          />
          <Item
            onClick={run(actions.metadados)}
            icon={I(
              "M19 3H5a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2V5a2 2 0 00-2-2zM9 17H7v-7h2v7zm4 0h-2V7h2v10zm4 0h-2v-4h2v4z",
            )}
            label={t("library.metadados")}
          />
          <Item
            onClick={run(actions.ocultar)}
            icon={I(
              "M12 7a5 5 0 015 5c0 .65-.13 1.26-.36 1.83l2.92 2.92A11.8 11.8 0 0023 12c-1.73-4.39-6-7.5-11-7.5-1.27 0-2.49.2-3.64.57l2.17 2.16C11.1 7.09 11.54 7 12 7zM2.71 3.16L1.39 4.47l2.17 2.17A11.77 11.77 0 001 12c1.73 4.39 6 7.5 11 7.5 1.55 0 3.03-.3 4.38-.84l3.15 3.15 1.31-1.31L2.71 3.16zM12 17a5 5 0 01-4.55-7.06l1.55 1.55A3 3 0 0012 15c.28 0 .55-.04.8-.11l1.55 1.55c-.72.36-1.51.56-2.35.56z",
            )}
            label={t("library.ocultar_jogo")}
          />
          <Item
            onClick={run(actions.favorito)}
            icon={I(
              game.favorite
            ...
            label={
              game.favorite ? t("library.remover_favoritos") : t("library.adicionar_favoritos")
            }
          />
          <Item
            ...
            label={t("library.categorias")}
          />
          {game.installed !== false && (
            <Item
              onClick={run(actions.desinstalar)}
              icon={I("M6 19a2 2 0 002 2h8a2 2 0 002-2V7H6v12zM19 4h-3.5l-1-1h-5l-1 1H5v2h14V4z")}
              label={t("library.desinstalar")}
            />
          )}
          {aviso && (
            <p className="border-t border-white/[0.07] px-4 py-2 text-[11px] leading-snug text-white/50">
              {aviso}
            </p>
          )}
        </>
