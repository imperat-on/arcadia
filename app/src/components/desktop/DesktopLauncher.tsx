...
// Shell do modo desktop (estilo Heroic): sidebar + área de conteúdo.
// Mouse-first: sem boot, sem gamepad, sem overlay de perfil.
export function DesktopLauncher() {
  ...
  useEffect(() => {
    carregar()
    // Aplica zoom/fontes/tema/toggles salvos (acessibilidade) ao abrir.
    ...
    // Recarrega a biblioteca quando um download termina ou um jogo é desinstalado.
    ...
    // Download da loja concluído: oferece restart da Steam (ou "mais tarde").
  ...
  return (
    <div className="flex h-screen w-full select-none overflow-hidden bg-black text-white antialiased">
      ...
      {/* Badge "Jogando <título>" no canto superior direito */}
      <PlayingBadge />

      {/* Download da loja concluído: reiniciar a Steam agora ou mais tarde */}
      {baixado && (
      ...
      {/* Confirmação: entrar no modo tela cheia (console) */}
      {confirmBigPicture && (
