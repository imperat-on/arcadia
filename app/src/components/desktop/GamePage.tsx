    ...
    >
      ...
      {g.hero || g.cover ? (
        <img
          src={g.hero || g.cover}
          alt=""
          className="pointer-events-none absolute inset-0 h-full w-full object-cover opacity-25 blur-md"
          draggable={false}
        />
      ) : null}
        ...
        <div className="relative flex min-h-0 flex-col overflow-hidden rounded-2xl border border-white/[0.08] bg-[#0d0d10]/90">
          <div className="relative h-[42%] shrink-0 overflow-hidden bg-black">
            ...
            {g.logo ? (
              <img
                src={g.logo}
                alt=""
                className="absolute left-5 top-5 max-h-[64px] max-w-[55%] object-contain object-left drop-shadow-[0_2px_6px_rgba(0,0,0,0.9)]"
                draggable={false}
              />
            ) : null}
          </div>
          ...
          <div className="flex min-h-0 flex-1 flex-col px-6 py-4">
            ...
            <div className="mt-4 shrink-0">
              ...
              {!instalado && (
                <p className="mt-1.5 text-[13px] italic" style={{ color: "var(--accent)" }}>
                  {t("gamepage.nao_instalado")}
                </p>
              )}
        ...
        <div className="flex min-h-0 flex-col overflow-y-auto rounded-2xl border border-white/[0.08] bg-[#0d0d10]/90 p-5">
            ...
            <div className="flex flex-col gap-4">
              <div className="flex items-center justify-between border-b border-white/[0.06] pb-3">
                ...
                <span className="text-[13px] text-white/70">
                  {sysBusy && !sys ? t("gamepage.carregando") : tamDownload}
                </span>
              </div>
              <div className="flex items-center justify-between border-b border-white/[0.06] pb-3">
                <span className="flex items-center gap-2.5 text-[13px] font-medium text-white/85">
                ...
                <span className="text-[13px] text-white/70">
                  {sysBusy && !sys ? t("gamepage.carregando") : tamInstalado}
                </span>
              </div>
              {sys?.version && (
                <div className="flex items-center justify-between border-b border-white/[0.06] pb-3">
                  ...
                  {sys.req_rec ? (
                    <div>
                      <h4
                        ...
                        style={{ color: "var(--accent)" }}
                      >
                        {t("gamepage.recomendados")}
                      </h4>
                      <p className="whitespace-pre-line leading-relaxed text-white/70">
                        {stripHtml(sys.req_rec)}
                      </p>
                    </div>
                  ) : null}
                  {g.launcher !== "steam" && (
                  ...
                  {(
                    [
                      ...
                      [
                        ...
                        g.metacritic != null ? `${g.metacritic}/100` : undefined,
                      ],
                    ] as const
                  ).map(([k, v]) => (
                    <div
                      key={String(k)}
                      className="flex items-center justify-between border-b border-white/[0.06] pb-2.5"
                    >
                      <span className="font-medium text-white/85">{k}:</span>
                      <span className="text-white/70">
                        {v != null && v !== "" ? String(v) : t("common.fallback")}
                      </span>
                    </div>
                  ))}
                  <p className="mt-2 text-[12px] text-white/35">
                    {t("gamepage.requisitos_indisponiveis")}
                  </p>
                </div>
              )}
            </div>
          ...
          {steamAppid && (
            <div className="mt-5 flex flex-col gap-5">
              ...
              {instalado && fixesAtivo && (
                <FixesPanel appid={steamAppid} installPath={installPath} />
              )}
