...
export function GameSettingsDialog({ game, onClose }: { game: Game; onClose: () => void }) {
  ...
  const Info = ({ id, label, texto }: { id: string; label: string; texto: string }) => (
    <div className="mb-2">
      ...
      >
        <span
          className="flex h-[15px] w-[15px] items-center justify-center rounded-full text-[10px] font-bold text-black"
      ...
      {ajuda === id && (
        <p className="ml-6 mt-1 text-[12px] leading-relaxed text-white/45">{texto}</p>
      )}
    </div>
  )
          ...
          <div className="min-h-0 overflow-y-auto px-5 py-4">
            {aba === "COMPATIBILIDADE" && (
              <>
                ...
                <Info
                  id="proton"
                  label={t("gamesettings.proton_ignorados")}
                  texto={t("gamesettings.proton_ignorados_info")}
                />
                <Info
                  id="caminho"
                  label={t("gamesettings.caminho_wine")}
                  texto={wineEscolhido || t("gamesettings.caminho_wine_info")}
                />
                <Info
                  id="ajuda_wine"
                  label={t("gamesettings.ajuda")}
                  texto={t("gamesettings.ajuda_info")}
                />

                <label className="mb-1.5 mt-4 block text-[13px] text-white/70">
                  {t("gamesettings.pasta_prefixo")}
                </label>
                <div className="mb-3 flex gap-2">
                  <input
                    ...
                    onChange={(e) =>
                      set({
                        prefixPath: e.target.value === defaultPrefix ? undefined : e.target.value,
                      })
                    }
                    spellCheck={false}
                ...
                <Info
                  id="pref"
                  label={t("gamesettings.pasta_prefixo")}
                  texto={t("gamesettings.prefixo_info")}
                />

                <div className="mt-4">
                  <Check
                    k="autoDXVK"
                    label={t("gamesettings.auto_dxvk")}
                    hint={t("gamesettings.auto_dxvk_hint")}
                  />
                  <Check
                    k="autoNVAPI"
                    label={t("gamesettings.auto_dxvk_nvapi")}
                    hint={t("gamesettings.auto_dxvk_nvapi_hint")}
                  />
                  <Check
                    k="autoVKD3D"
                    label={t("gamesettings.auto_vkd3d")}
                    hint={t("gamesettings.auto_vkd3d_hint")}
                  />
                  <Check
                    k="esync"
                    label={t("gamesettings.esync")}
                    hint={t("gamesettings.esync_hint")}
                  />
                  <Check
                    k="fsync"
                    label={t("gamesettings.fsync")}
                    hint={t("gamesettings.fsync_hint")}
                  />
                  <Check
                    k="wineWayland"
                    label={t("gamesettings.wine_wayland")}
                    hint={t("gamesettings.wine_wayland_hint")}
                  />
                  <Check
                    k="wow64"
                    label={t("gamesettings.wow64")}
                    hint={t("gamesettings.wow64_hint")}
                  />
                  <Check
                    k="fsrHack"
                    label={t("gamesettings.fsr_hack")}
                    hint={t("gamesettings.fsr_hack_hint")}
                  />
                </div>
            ...
            {aba === "DESEMPENHO" && (
              <>
                <Check
                  k="gamemode"
                  label={t("gamesettings.gamemode")}
                  hint={t("gamesettings.gamemode_hint")}
                />
                <Check
                  k="mangohud"
                  label={t("gamesettings.mangohud")}
                  hint={t("gamesettings.mangohud_hint")}
                />
                <label className="mb-1.5 mt-2 block text-[13px] text-white/70">
                  {t("gamesettings.dxvk_hud")}
                </label>
                <select
                  value={s.dxvkHud || ""}
            ...
            {aba === "GERAL" && (
              <>
                ...
                <label className="mb-1.5 block text-[13px] text-white/70">
                  {t("gamesettings.opcoes_inicializacao")}
                </label>
                <input
                  value={s.gameArgs || ""}
                ...
                <div className="mt-2">
                  <Info
                    id="args"
                    label={t("gamesettings.ajuda")}
                    texto={t("gamesettings.argumentos_info")}
                  />
                </div>
              </>
            )}
            ...
            {aba === "AVANÇADO" && (
              <>
                <Check
                  k="verboseLogs"
                  label={t("gamesettings.verbose_logs")}
                  hint={t("gamesettings.verbose_logs_hint", { id: game.id })}
                />

                <h3 className="mb-2 mt-5 text-[13px] font-medium text-white/80">
                  {t("gamesettings.scripts")}
                </h3>
                {(
                  [
                      ...
                      <div className="flex-1 truncate rounded-lg border border-white/10 bg-white/[0.04] px-3.5 py-2.5 text-[13px] text-white/60">
                        {(s[k] as string) || (
                          <span className="text-white/25">
                            {t("gamesettings.script_placeholder")}
                          </span>
                        )}
                      </div>
                      {s[k] ? (
                ...
                <h3 className="mb-2 mt-2 text-[13px] font-medium text-white/80">
                  {t("gamesettings.wrapper")}
                </h3>
                {(s.wrappers || []).length > 0 && (
                  <>
                    <div
                      className="mb-1 grid grid-cols-[1fr_1fr_28px] gap-2 text-[12px] font-semibold"
                      style={{ color: "var(--accent)" }}
                    >
                      <span>{t("gamesettings.wrapper_cabecalho")}</span>
                      <span>{t("gamesettings.wrapper_argumentos")}</span>
                      <span />
                    </div>
                    {(s.wrappers || []).map((w, i) => (
                      <div
                        key={i}
                        className="mb-1.5 grid grid-cols-[1fr_1fr_28px] items-center gap-2"
                      >
                        <span className="truncate rounded-lg border border-white/10 bg-white/[0.04] px-3 py-2 font-mono text-[12px] text-white/80">
                          {w.cmd}
                        </span>
                        <span className="truncate rounded-lg border border-white/10 bg-white/[0.04] px-3 py-2 font-mono text-[12px] text-white/60">
                          {w.args || "—"}
                        </span>
                        <button
                          onClick={() =>
                            set({ wrappers: (s.wrappers || []).filter((_, j) => j !== i) })
                          }
                ...
                <div className="grid grid-cols-[1fr_1fr_28px] items-center gap-2">
                  ...
                  <button
                    onClick={() => {
                      ...
                      if (!cmd) return
                      set({
                        wrappers: [...(s.wrappers || []), { cmd, args: novoWrap.args.trim() }],
                      })
                ...
                <div className="mt-2">
                  <Info
                    id="wrap"
                    label={t("gamesettings.ajuda")}
                    texto={t("gamesettings.wrapper_ajuda")}
                  />
                </div>

                <h3 className="mb-2 mt-5 text-[13px] font-medium text-white/80">
                  {t("gamesettings.env_vars")}
                </h3>
                {(s.envVars || []).length > 0 && (
                  <>
                    <div
                      className="mb-1 grid grid-cols-[1fr_16px_1fr_28px] gap-2 text-[12px] font-semibold"
                      style={{ color: "var(--accent)" }}
                    >
                      <span>{t("gamesettings.env_nome")}</span>
                      <span />
                      <span>{t("gamesettings.env_valor")}</span>
                      <span />
                    </div>
                    {(s.envVars || []).map((v, i) => (
                      <div
                        key={i}
                        className="mb-1.5 grid grid-cols-[1fr_16px_1fr_28px] items-center gap-2"
                      >
                        <span className="truncate rounded-lg border border-white/10 bg-white/[0.04] px-3 py-2 font-mono text-[12px] text-white/80">
                          {v.name}
                        </span>
                        ...
                        <button
                          onClick={() =>
                            set({ envVars: (s.envVars || []).filter((_, j) => j !== i) })
                          }
                          title={t("gamesettings.remover")}
                          className="flex h-7 w-7 items-center justify-center rounded-md text-white/40 transition-colors hover:bg-white/10 hover:text-white"
                        >
                ...
                <div className="grid grid-cols-[1fr_16px_1fr_28px] items-center gap-2">
                  <input
                    ...
                    className={`rounded-lg border bg-white/[0.04] px-3 py-2 text-[12px] text-white outline-none placeholder:text-white/25 focus:border-[color:var(--accent)] ${
                      novaVar.name && !/^[A-Za-z_][A-Za-z0-9_]*$/.test(novaVar.name)
                        ? "border-red-500/60"
                        : "border-white/10"
                    }`}
                  />
                  ...
                  <button
                    onClick={() => {
                      ...
                      set({
                        envVars: [
                          ...(s.envVars || []).filter((v) => v.name !== name),
                          { name, value: novaVar.value },
                        ],
                      })
                ...
                <div className="mt-2">
                  <Info
                    id="env"
                    label={t("gamesettings.ajuda")}
                    texto={t("gamesettings.env_ajuda")}
                  />
                </div>
              </>
            )}
            ...
            {aba === "AVANÇADO" && (
              <>
                <h3 className="mb-2 mt-6 text-[13px] font-medium text-white/80">
                  {t("gamesettings.legado_titulo")}
                </h3>
